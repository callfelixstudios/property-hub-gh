...
export default function PriceDisplay({ 
  ...
  const upfrontPrice = showUpfront ? formatPrice(rawPrice * rentAdvanceMonths, currency) : null;
  
  return (
    <div className="flex flex-col">
      <div className="text-lg font-extrabold text-navy-base">
        {formattedPrice}
        {priceSuffix && <span className="text-xs font-normal text-gray-500 ml-1">{priceSuffix}</span>}
      </div>
      {showUpfront && (
