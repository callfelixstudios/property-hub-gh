...
export async function generateMetadata({
  ...
  const data = await getLocationData(location, 'sale');
  if (!data) {
    return { title: 'Sales Not Found | Property Hub GH' };
  }

  const locationName = data.ref.neighborhood || formatRegionForUi(data.ref.region);
  const phrase = categoryPhrase(data.dominantCategory, 'sale');
  ...
  const pageUrl = `${SITE_URL}/sales/${data.ref.slug}`;
...
export default async function SalesLocationPage({
  ...
  const phrase = categoryPhrase(dominantCategory, 'sale');
  const listingWord = stats.count === 1 ? 'listing' : 'listings';

  const filterUrl =
    ref.kind === 'neighborhood'
      ? `/sales?region=${encodeURIComponent(displayRegion)}&neighborhood=${encodeURIComponent(
          ref.neighborhood ?? ''
        )}`
    ...
    <div className="w-full min-h-screen bg-surface-primary flex flex-col">
      <JsonLd
        data={getBreadcrumbSchema([
          ...
          { name: locationName, url: `/sales/${ref.slug}` },
        ])}
      />
      <JsonLd
        data={{
      ...
      <div className="bg-navy-base pt-28 pb-10 px-4 mb-8">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/80 max-w-2xl">
            {stats.count} active sale {listingWord} in {locationName} — verified on Property Hub
            GH.
          </p>
        </div>
      </div>
